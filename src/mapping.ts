...
function handleCubanDepositTemplate(
  event: Deposit,
  amount: BigInt,
  amountInUSD: BigDecimal,
  accountId: string,
...
function handleCubanWithdrawalTemplate(
  event: Withdraw,
  amount: BigInt,
  amountInUSD: BigDecimal,
  accountId: string,
...
/** Cuban's Ape Strategy  **/

export function handleCubanShareTransfer(event: Transfer): void {
  let transactionId = event.address
    .toHexString()
    .concat("-")
    .concat(event.transaction.hash.toHexString())
  ...
  let farmer = getOrCreateCubanFarmer(event.address);
  farmer.underlyingToken = getOrCreateToken(event.address).id; // Added deposit token
  let fromAccount = getOrCreateAccount(event.params.from.toHexString());
  let toAccount = getOrCreateAccount(event.params.to.toHexString());
  let shareToken = getOrCreateToken(Address.fromString(farmer.shareToken));
...
export function handleCubanDeposit(event: Deposit): void {
  let transactionId = event.address
    .toHexString()
    .concat("-")
    .concat(event.transaction.hash.toHexString())
...
export function handleCubanWithdraw(event: Withdraw): void {
  let transactionId = event.address
    .toHexString()
    .concat("-")
    .concat(event.transaction.hash.toHexString())
